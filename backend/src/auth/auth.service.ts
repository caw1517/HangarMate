...
export class AuthService {
    constructor(private prisma: PrismaService, private jwt: JwtService, private config: ConfigService, private userService: UsersService) {}
    ...
    async Login(email: string, password: string) {
        ...
        if (user) {
            const pwMatches: boolean = await argon.verify(user.hashedPassword, password);
